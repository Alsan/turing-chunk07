...
import org.wso2.carbon.automation.core.utils.frameworkutils.productsetters.*;
...
public class FrameworkFactory {
    ...
    public static FrameworkProperties getFrameworkProperties(String product) {
        ...
        if (product.equals(ProductConstant.UES_SERVER_NAME)) {
            UesSetter uesSetter = new UesSetter();
            properties.setDataSource(uesSetter.getDataSource());
            properties.setEnvironmentSettings(uesSetter.getEnvironmentSettings());
            properties.setEnvironmentVariables(uesSetter.getEnvironmentVariables());
            properties.setRavana(uesSetter.getRavana());
            properties.setSelenium(uesSetter.getSelenium());
            properties.setProductVariables(uesSetter.getProductVariables());
            properties.setWorkerVariables(uesSetter.getWorkerVariables());
        }
