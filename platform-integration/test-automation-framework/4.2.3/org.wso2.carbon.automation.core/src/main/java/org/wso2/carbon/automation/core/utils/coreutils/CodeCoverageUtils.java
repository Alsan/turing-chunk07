...
public final class CodeCoverageUtils {
    ...
    private static void instrumentSelectedFiles(String carbonHome) throws IOException {
        ...
        //instrument the jar files are patches directories
        int instrumentedPatchFileCount = 0;
        int numberOfPatches = patchesDir.listFiles().length;
        for (int i = 1; i <= numberOfPatches; i++) {
            String stringRepOfInt = String.format("%04d", i);
            File patchFile = new File(patchesDir.getAbsolutePath() + File.separator + "patch" + stringRepOfInt);
            log.info("Instrumenting jar files at : " + patchFile.getName());
            Collection patchesCollection = FileUtils.listFiles(patchFile, null, true);
