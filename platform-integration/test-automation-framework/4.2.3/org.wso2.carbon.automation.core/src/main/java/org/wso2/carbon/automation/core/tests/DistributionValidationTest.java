...
import org.wso2.carbon.automation.core.tests.utils.DistributionValidationUtils;
...
public abstract class DistributionValidationTest {
    ...
    private static final DistributionValidationUtils utils = new DistributionValidationUtils();
    ...
    public void init() throws Exception {
        ...
        reportFile = new File(ProductConstant.getReportLocation() + File.separator
        ...
        utils.readLicenseFile(productPath, licenceJarList, duplicateJarList);
    ...
    public void testValidateJarFilesAgainstDistribution() throws Exception {
        ...
        utils.reportGeneratorList(additionalJarFilesInLicenceFile, "Following jar files mentioned" +
    ...
    public void testValidateJarFilesAgainstLicenceFile() throws Exception {
        ...
        utils.reportGeneratorList(jarsNotMentionedInLicenceFile, "Following jar files inside the " +
    ...
    public void testValidateDuplicateJarEntriesInLicenceFile() throws Exception {
        ...
        if (duplicateJarList.size() > 0) {
            ...
            utils.reportGeneratorList(new ArrayList<String>(duplicateJarList), "Following are the " +
                    "duplicate jar file list in the LICENSE.txt file", reportFile);

        }

        assertFalse("Duplicate jar entries exist in LICENSE.txt", duplicateJars);
    }
    ...
    public void testRecursiveScanForSnapshotKeyword() throws Exception {
        ...
        List<File> distributionTextualFileList = utils.recursiveScanToRetrieveTextualFiles
        ...
        utils.reportGeneratorList(textualFileNameWithSnapshotKeyword, "Following are the" +
                " textual file names which contain " + KEYWORD + " keyword", reportFile);

        utils.reportGeneratorMap(snapshotKeywordMap, "List of textual file names together" +
                " with number of occurrences of " + KEYWORD + " keyword", reportFile);

        utils.reportGeneratorList(jarFileNameWithSnapshotKeyword, "List of jar file " +
    ...
    public void testMavenVariablesReplacement() throws Exception {
        ...
        for (String xmlFile : xmlFileList) {
            ...
            Collection files = FileUtils.listFiles(new File(productPath + File.separator
                    + "repository" + File.separator + "conf"), null, recursive);
            ...
            utils.validateXml(xsdValidateMap, pathToXML, (getPathToXSD() + File.separator +
                    new File(pathToXML).getName().replace(".xml", ".xsd")));
        }

        utils.reportGeneratorMap(xsdValidateMap, "Following are the configuration file " +
    ...
    public void testCompareDistributionSize() throws Exception {
        ...
        List<String> linksToVersion = utils.getLinks(urlToDistributionList);
        List<String> linksFromVersion = utils.getLinks(linksToVersion.get(linksToVersion.size() - 1));
        ...
        if (Double.parseDouble(difference) >= 20) {
            ...
            utils.reportGeneratorMap(resultsDataMap, "Running distribution size " +
                    "comparison failure.Below are the sizes of two packs compared.", reportFile);

            sizeDifferenceStatus = true;
        } else {
            utils.reportGeneratorMap(resultsDataMap, "Running distribution size comparison passed." +
    ...
    public void testIdentifyingDuplicateJarFiles() throws Exception {
        ...
        Set<String> duplicateSet = utils.getDuplicateJarSet(jarFileListInDistribution);

        //check different directories duplicate jars
        utils.identifyDuplicateJars(jarFileListInDistribution, distributionVersion,
                distributionDuplicateJarList, unidentifiedVersionJars);

        utils.reportGeneratorList(new ArrayList<String>(duplicateSet),
                "These are duplicate jar files in the same directory", reportFile);
        utils.reportGeneratorList(new ArrayList<String>(distributionDuplicateJarList),
                "These jar files were identified as duplicates", reportFile);
        utils.reportGeneratorList(new ArrayList<String>(unidentifiedVersionJars),
    ...
    public void testSamplesDirectoryStructureSrcValidation() throws Exception {
        ...
        if (new File(samplesDirPath).exists()) {
            versionArr = utils.validateSamplesDirectoryStructureIdentifyVersions(samplesDirPath,
                    versionArr);
            utils.reportGeneratorList(versionArr, "Following are the sample directories " +
                    "with versions embedded", reportFile);
        } else {
            utils.reportGeneratorList(versionArr, "No sample directory detected.", reportFile);
        }

        utils.searchDirectoryByName(samplesDirPath, directoryLists, "/src");
        ...
        utils.reportGeneratorMap(structureViolation, "Following are the maven directory " +
    ...
    public void testSamplesDirectoryStructureResourcesValidation() throws Exception {
        ...
        if (new File(samplesDirPath).exists()) {
            ...
            for (String dirName : directoryLists) {
                utils.searchDirectoryByName(dirName, tempArrList, "/resources");
        ...
        utils.reportGeneratorMap(resourcesStructureViolation, "Following are the src " +
