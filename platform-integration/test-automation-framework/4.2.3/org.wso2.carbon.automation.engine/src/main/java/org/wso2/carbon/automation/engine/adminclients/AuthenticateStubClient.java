...
public class AuthenticateStubClient {
    private static final Log log = LogFactory.getLog(AuthenticateStubClient.class);

    /**
     * @param sessionCookie session cookie of the session
     * @param stub          stub that should be authenticated
     */
    ...
    public static Stub authenticateStub(Stub stub, String sessionCookie, String backendURL) {
        ...
        option.setProperty(org.apache.axis2.transport.http.HTTPConstants.COOKIE_STRING,
                sessionCookie);
        option.setTo(new EndpointReference(backendURL +
                client.getServiceContext().getAxisService().getName()
                .replaceAll("[^a-zA-Z]", "")));
        if (log.isDebugEnabled()) {
            log.debug("AuthenticateStub : Stub created with session " + sessionCookie);
        }
