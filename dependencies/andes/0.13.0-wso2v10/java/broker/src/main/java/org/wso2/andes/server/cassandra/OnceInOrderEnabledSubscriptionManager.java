...
public class OnceInOrderEnabledSubscriptionManager implements ClusteringEnabledSubscriptionManager{
    ...
    public void init() {
        ThreadFactory namedThreadFactory = new ThreadFactoryBuilder().setNameFormat("OnceInOrderSubscriptionManager-%d").build();

        onceInOrderExecutor = Executors.newFixedThreadPool(ClusterResourceHolder.getInstance().getClusterConfiguration().
   ...
   public Map<AMQChannel, Map<Long, Semaphore>> getUnAcknowledgedMessageLocks() {
    ...
    public List<String> getNodeQueuesHavingSubscriptionsForQueue(String destinationQueue) {
        //if once in order support enabled we do not want global queue workers to copy messages anywhere
        return null;
    }

    private void start() {
        active = true;
        onceInOrderExecutor.submit(new CassandraReliableMessageFlusherManagerTask());
    }
                ...
                if (subscriptionQueue.size() > 0) {
                    ...
                    if (workMap.containsKey(id)) {
                        ...
                        if (work.isMarkedForRemoval()) {
                            ...
                            if (!work.isWorking()) {
                                onceInOrderExecutor.execute(work);
                            }
                            subscriptionQueue.remove();
