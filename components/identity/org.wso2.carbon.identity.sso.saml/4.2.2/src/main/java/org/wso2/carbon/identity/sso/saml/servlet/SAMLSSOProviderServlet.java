...
public class SAMLSSOProviderServlet extends HttpServlet {
    ...
    private void handleRequestFromLoginPage(HttpServletRequest req, HttpServletResponse resp,
        ...
        if(samlSSOService.isOpenIDLoginAccepted() && req.getSession().getAttribute("authenticatedOpenID") != null){
            ...
            authRespDTO = samlSSOService.authenticate(authnReqDTO, sessionId, true, SAMLSSOConstants.AuthnModes.OPENID);
        } else {
            authRespDTO = samlSSOService.authenticate(authnReqDTO, sessionId, (Boolean)req.getAttribute("commonAuthAuthenticated"), SAMLSSOConstants.AuthnModes.USERNAME_PASSWORD);
        }
