...
public class CommonApplicationAuthenticationServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		if (request.getParameter(ApplicationAuthenticatorConstants.RequestParams.TYPE) != null) {
	...
	private boolean handleInitialRequest(HttpServletRequest request, HttpServletResponse response) 
		...
		String callerSessionDataKey = request.getParameter(ApplicationAuthenticatorConstants.SESSION_DATA_KEY);
		
		//"commonAuthCallerPath" - path of the calling servlet. This is the url response should be sent to
		String callerPath = URLDecoder.decode(request.getParameter(ApplicationAuthenticatorConstants.RequestParams.CALLER_PATH), "UTF-8");
		
		//"type" - type of the request. e.g. samlsso, openid, oauth, passivests
		String requestType = request.getParameter(ApplicationAuthenticatorConstants.RequestParams.TYPE);
	...
	private void cleanUpSession(HttpServletRequest request) {
		...
		request.getSession().removeAttribute(ApplicationAuthenticatorConstants.SESSION_DATA_KEY);
	}
}