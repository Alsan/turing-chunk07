...
public class SAMLSSOAuthenticator extends AbstractApplicationAuthenticator {
    ...
    public AuthenticatorStatus authenticate(HttpServletRequest request, HttpServletResponse response, ApplicationAuthenticationContext context) {
    	...
    	Map<String, String> parameterMap = getIdPConfigs(request, context).getParameterMap();
    	
    	try {
	        new SAMLSSOManager(parameterMap).processResponse(request, parameterMap);
	...
	public void sendInitialRequest(HttpServletRequest request, HttpServletResponse response, ApplicationAuthenticationContext context) {
		...
		String idpURL = parameterMap.get("IdPLoginURL");
		String loginPage = "";
		
        try {
	        loginPage = new SAMLSSOManager(parameterMap).buildRequest(request, false, false, idpURL, parameterMap, context.getContextIdentifier());
        } catch (SAMLSSOException e) {
    ...
    public AuthenticatorStatus logout(HttpServletRequest request, HttpServletResponse response, ApplicationAuthenticationContext context) {
		...
		Map<String, String> parameterMap = getIdPConfigs(request, context).getParameterMap();
		String idpLogoutURL = parameterMap.get("IdPLogoutURL");
		String loginPage = "";
		
        try {
	        loginPage = new SAMLSSOManager(parameterMap).buildRequest(request, true, false, idpLogoutURL, parameterMap, context.getContextIdentifier());
        } catch (SAMLSSOException e) {
        	log.error("Exception while building the SAMLRequest", e);
        }
