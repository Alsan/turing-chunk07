        ...
        if(domainMappingEnabled && !tlbMembershipHandler.isAValidHostName(targetHost)) {
            ...
            if (domainMapping != null) {
                ...
                if(actualHost.contains("/") && containsPort ) {
                	 transportHeaders.put(HTTP.TARGET_HOST,
                                          actualHost.substring(0,actualHost.indexOf("/"))+ ":" +
                                          port+actualHost.substring(actualHost.indexOf("/"))) ;
                ...
                ((Axis2MessageContext) synCtx).getAxis2MessageContext().
                        setProperty("TRANSPORT_HEADERS" , transportHeaders);
...
//    }
    ...
    private int getTenantId(String url) {
        ...
        if (url != null && url.contains(servicesPrefix)) {
            ...
            if (domainName != null) {
                try {
                    return ConfigHolder.getInstance().getRealmService().
                            getTenantManager().getTenantId(domainName);
    ...
    private class TenantDynamicLoadBalanceFaultHandlerImpl extends DynamicLoadbalanceFaultHandler {
        ...
        public void onFault(MessageContext synCtx) {
            ...
            log.info("Failed to send message to Member " + currentMember + " . Error Code: "+errorCode);
            if (errorCode != null) {
                ...
                if (errorCode.equals(NhttpConstants.CONNECTION_FAILED) ||
                    errorCode.equals(NhttpConstants.CONNECTION_TIMEOUT)) {

                    if (!synCtx.getFaultStack().isEmpty()) {
                        synCtx.getFaultStack().pop();
                    }
                    // Try to resend to another member
                    Member newMember = tlbMembershipHandler.
                            getNextApplicationMember(host, getTenantId(synCtx.toString()));
                ...
                } else if (errorCode.equals(NhttpConstants.CONNECT_TIMEOUT)) {
                    String msg = "Error sending request! " +
                                 "Connection timeout after request is read by the host "+host
                                 +" . Error code: " +errorCode+ " Protocol State: " + "RESPONSE HEAD" ;
                    log.error(msg);
                    throw new SynapseException(msg);
                } else if (errorCode.equals(NhttpConstants.CONNECT_CANCEL)) {
                    String msg = "Error sending request!" +
                                 " Connection timeout after request is read by the host " + host +
                                 " . Error code: " + errorCode+ " Protocol State: " + "REQUEST DONE";
                    log.error(msg);
                    throw new SynapseException(msg);
