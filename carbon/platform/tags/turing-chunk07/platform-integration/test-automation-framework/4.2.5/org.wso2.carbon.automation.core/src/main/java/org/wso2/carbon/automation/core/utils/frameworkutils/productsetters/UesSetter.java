...
public class UesSetter extends EnvironmentSetter {
    ...
    public UesSetter() {
        ...
        if (Boolean.parseBoolean(prop.getProperty("stratos.test"))) {
            hostNames = (properties.getProperty("ues.service.host.name", "ues.stratoslive.wso2.com"));
        } else {
            hostNames = (properties.getProperty("ues.host.name", "localhost"));
        }
        String httpPorts = (prop.getProperty("ues.http.port", "9763"));
        String httpsPorts = (prop.getProperty("ues.https.port", "9443"));
        String webContextRoots = (prop.getProperty("ues.webContext.root", "admin"));
        String nHttpPorts = (prop.getProperty("ues.nhttp.port", "8280"));
        String nHttpsPorts = (prop.getProperty("ues.nhttps.port", "8243"));
