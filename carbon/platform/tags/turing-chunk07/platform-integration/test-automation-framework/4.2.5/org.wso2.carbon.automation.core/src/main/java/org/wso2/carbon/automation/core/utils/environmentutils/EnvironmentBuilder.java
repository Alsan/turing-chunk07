...
public class EnvironmentBuilder {
    ...
    protected EnvironmentVariables ues;
    ...
    public EnvironmentBuilder ues(int userId)
            throws LoginAuthenticationExceptionException, RemoteException {
        envVariables = new EnvironmentVariables();
        FrameworkProperties frameworkProperties =
                FrameworkFactory.getFrameworkProperties(ProductConstant.UES_SERVER_NAME);
        ProductVariables uesSetter = frameworkProperties.getProductVariables();
        WorkerVariables uesWorker = frameworkProperties.getWorkerVariables();
        AuthenticatorClient managerServiceAuthentication;
        AuthenticatorClient workerServiceAuthentication = null;

        managerBackEndUrl = uesSetter.getBackendUrl();
        workerBackEndUrl = uesWorker.getBackendUrl();
        managerHostName = uesSetter.getHostName();
        workerHostName = uesWorker.getHostName();
        httpPort = uesSetter.getHttpPort();
        managerServiceAuthentication = new AuthenticatorClient(managerBackEndUrl);

        envVariables = loginSetup(userId, managerServiceAuthentication, workerServiceAuthentication
                , frameworkProperties, uesSetter, uesWorker);
        this.ues = envVariables;
        return this;
    }
    ...
    public EnvironmentBuilder am(int tenent)
            throws RemoteException, LoginAuthenticationExceptionException {
        envVariables = new EnvironmentVariables();
        AuthenticatorClient managerServiceAuthentication;
        AuthenticatorClient workerServiceAuthentication = null;
        FrameworkProperties frameworkProperties =
                FrameworkFactory.getFrameworkProperties(ProductConstant.AM_SERVER_NAME);
        ProductVariables amSetter = frameworkProperties.getProductVariables();
        WorkerVariables amWorker = frameworkProperties.getWorkerVariables();
        managerBackEndUrl = amSetter.getBackendUrl();
        workerBackEndUrl = amWorker.getBackendUrl();
        managerHostName = amSetter.getHostName();
        workerHostName = amWorker.getHostName();
        httpPort = amSetter.getHttpPort();
        managerServiceAuthentication = new AuthenticatorClient(managerBackEndUrl);
        if (frameworkProperties.getEnvironmentSettings().isClusterEnable()) {
            workerServiceAuthentication = new AuthenticatorClient(workerBackEndUrl);
        }
        envVariables = loginSetup(tenent, managerServiceAuthentication, workerServiceAuthentication,
                                  frameworkProperties, amSetter, amWorker);
        this.am = envVariables;
        return this;
    }
    ...
    private String getServiceURL(FrameworkProperties frameworkProperties,
                                 ProductVariables productSetter, WorkerVariables workerVariables,
                                 UserInfo userInfo) {
    ...
    private String getSecureServiceURL(FrameworkProperties frameworkProperties,
                                       ProductVariables productSetter,
                                       WorkerVariables workerVariables, UserInfo userInfo) {
        String generatedSecureServiceURL;
        if (frameworkProperties.getEnvironmentSettings().isClusterEnable()) {
