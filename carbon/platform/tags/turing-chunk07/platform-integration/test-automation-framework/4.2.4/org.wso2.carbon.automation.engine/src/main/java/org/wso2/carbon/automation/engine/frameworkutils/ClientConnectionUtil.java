...
public class ClientConnectionUtil {
    ...
    public static void waitForLogin()
        ...
        UserManagerContext userManagerContext = InitialContextPublisher.getContext().getUserManagerContext();
        Tenant superAdmin = userManagerContext.getTenantsNode(FrameworkConstants.SUPER_TENANT_DOMAIN_NAME)
                .getAdminTenant();
        AutomationContext automationContext = InitialContextPublisher.getContext();
        String executionEnvironment =
                automationContext.getConfigurationContext().getConfiguration().getExecutionEnvironment();
        if (executionEnvironment.equals(Platforms.product.name())) {
            ...
            for (Instance instance : instanceList) {
                ...
                String backendURL = InitialContextPublisher
                ...
                while (loginFailed && (System.currentTimeMillis() - startTime) < TIMEOUT) {
                    ...
                    try {
                        new LoginLogoutUtil(backendURL).login
                                (FrameworkConstants.SUPER_TENANT_DOMAIN_NAME, superAdmin.getUserName(),
