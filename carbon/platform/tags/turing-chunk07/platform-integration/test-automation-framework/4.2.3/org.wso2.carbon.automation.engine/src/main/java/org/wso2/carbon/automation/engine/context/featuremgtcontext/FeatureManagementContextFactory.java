...
public class FeatureManagementContextFactory {
    ...
    public void createFeatureManagementContext(OMElement nodeElement) {
        LinkedHashMap<String, P2Repositories> p2RepositoriesMap = new LinkedHashMap<String, P2Repositories>();
        OMElement node;
        ...
        while (children.hasNext()) {
            ...
            while (repoList.hasNext()) {
                ...
                String repoId = ((OMElementImpl) repositoryNode).getAttributeValue
                        (QName.valueOf(ContextConstants.FEATURE_MANAGEMENT_CONTEXT_REPOSITORY_ID));
                String url = ((OMElementImpl) repositoryNode).getText();
                repository.setName(repoId);
