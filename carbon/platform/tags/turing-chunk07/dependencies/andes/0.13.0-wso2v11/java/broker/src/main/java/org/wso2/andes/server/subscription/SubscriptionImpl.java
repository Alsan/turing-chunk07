                ...
                recordMessageDelivery(entry, deliveryTag);
                    ...
                    for (int i = 0; i < retryCount; i++) {
                        try {
                            ...
                            if (((AMQMessage)entry.getMessage()).isTopicMessage() 
...
/*        ClusterResourceHolder.getInstance()
                .getCassandraMessageStore().addAckedMessage(queueEntry);*/
        MessagingEngine.getInstance().ackReceived(new AndesAckData(queueEntry.getMessage().getMessageNumber(),queueEntry.getQueue().getName(),queueEntry.getQueue().checkIfBoundToTopicExchange()));
    }
