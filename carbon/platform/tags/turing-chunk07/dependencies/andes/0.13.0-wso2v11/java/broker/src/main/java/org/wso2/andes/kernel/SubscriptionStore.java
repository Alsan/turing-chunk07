...
public class SubscriptionStore {
	...
	public SubscriptionStore(DurableStoreConnection connection) throws AndesException {
        ...
        try {
        this.keyspace = ((CassandraConnection)connection).getKeySpace();
        this.cluster =  ((CassandraConnection)connection).getCluster();

        //create needed column families
        CassandraDataAccessHelper.createColumnFamily(SUBSCRIPTIONS_COLUMN_FAMILY, KEYSPACE, this.cluster, UTF8_TYPE);
    ...
    public void reloadSubscriptionsFromStorage(){
    	Map<String, List<String>> results = CassandraDataAccessHelper.listAllStringRows(SUBSCRIPTIONS_COLUMN_FAMILY, keyspace);
    ...
    private void createDisconnectOrRemoveLocalSubscription(LocalSubscription subscrption, SubscriptionChange type) throws AndesException {
        try {
            ...
            if (type ==SubscriptionChange.Added || type ==SubscriptionChange.Disconnected) {
                ...
                CassandraDataAccessHelper.addMappingToRaw(SUBSCRIPTIONS_COLUMN_FAMILY, destinationStringForCassandra, subscriptionID, subscrption.encodeAsStr(), keyspace);
    ...
    private LocalSubscription removeLocalSubscriptionAndNotify(String destination, String subscriptionID) throws AndesException{
    	...
    	try {
			...
			if(subscriptionToRemove != null){
				...
				CassandraDataAccessHelper.deleteStringColumnFromRaw(SUBSCRIPTIONS_COLUMN_FAMILY, destinationAtCassandra, subscriptionID, keyspace);
    ...
    private void addMessageCounterForQueue(String destinationQueueName) throws CassandraDataAccessException {
        CassandraDataAccessHelper.insertCounterColumn(MESSAGE_COUNTERS_COLUMN_FAMILY, MESSAGE_COUNTERS_RAW_NAME,destinationQueueName,keyspace);
    }

    private void removeMessageCounterForQueue(String destinationQueueName) throws CassandraDataAccessException {
        CassandraDataAccessHelper.removeCounterColumn(MESSAGE_COUNTERS_COLUMN_FAMILY, MESSAGE_COUNTERS_RAW_NAME, destinationQueueName, keyspace);
    }
    ...
    public void createExchange(AndesExchange exchange) throws AndesException{
        try {
            ...
            CassandraDataAccessHelper.addMappingToRaw(EXCHANGE_COLUMN_FAMILY, EXCHANGE_ROW, exchange.exchangeName, value, keyspace);
        } catch (Exception e) {
    ...
    public void deleteExchage(AndesExchange exchange) throws AndesException{
        try {
            CassandraDataAccessHelper.deleteStringColumnFromRaw(EXCHANGE_COLUMN_FAMILY, EXCHANGE_ROW, exchange.exchangeName, keyspace);
        } catch (Exception e) {
