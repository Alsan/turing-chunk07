            ...
            if (working) {
                try {
                    ...
                    if (messages != null && messages.size() > 0) {
                        List<AndesRemovableMetadata> publishedMessages = new ArrayList<AndesRemovableMetadata>();
                        for (AndesMessageMetadata message : messages) {
                            try {
                                enqueueMessage(message);
                                publishedMessages.add(new AndesRemovableMetadata(message.getMessageID(), message.getDestination()));
                        ...
                        messageStore.deleteMessageMetadataFromQueue(queueAddress, publishedMessages);
                    } else {
                        try {
