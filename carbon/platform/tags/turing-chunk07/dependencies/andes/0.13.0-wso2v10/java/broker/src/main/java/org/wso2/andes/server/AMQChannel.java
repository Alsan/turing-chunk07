...
public class AMQChannel implements SessionConfig, AMQSessionModel
{
            ...
            throws AMQException
    {
        ...
        _messageQueue = new CassandraQueue();
            ...
            throws AMQException
    {
        ...
        else
        {
          ...
          /*  final StoredMessage<MessageMetaData> handleCassandra =  cassandraMessageStore.addMessage(mmd);
            _currentMessage.set_storedCassandraMessageHandle(handleCassandra);*/
        ...
        if (_currentMessage.allContentReceived())
        {
            try
            {
                ...
                try {
                     ...
                     * happen here
                     */
                    DisruptorBasedExecutor disruptorExecutor = ClusterResourceHolder.getInstance().
                            getCassandraMessageStore().getDisruptorBasedExecutor();

                    MessageMetaData metaData = incomingMessage.getStoredMessage().getMetaData();
                    ...
                    metadata.setMessageID(incomingMessage.getMessageNumber());
                    metadata.setMetadata(underlying);
                    metadata.setQueue(queue);

                    if(queue == null){
                        _logger.error("Queue cannot be null, for "+ incomingMessage.getMessageNumber());
                    }

                    disruptorExecutor.messageCompleted(metadata, _channelId);
...
//            _messageQueue.addMessageContent(contentChunk.getData().buf());
                                           ...
                                           FieldTable filters, boolean noLocal, boolean exclusive) throws AMQException
    {
        ...
        ClusteringEnabledSubscriptionManager subscriptionManager = ClusterResourceHolder.
                getInstance().getSubscriptionManager();
        try {
            queue.registerSubscription(subscription, exclusive);
            CassandraSubscription cs = new CassandraSubscription(subscription, _session , queue.getResourceName());
...
//            buf.append(entry.getMessage().getMessageNumber()).append(" ");
        }
...
//        StringBuffer buf = new StringBuffer();
//        for(QueueEntry entry: ackedMessageMap.values()){
//            buf.append(entry.getMessage().getMessageNumber()).append(" ");
//        }

        if(!ackedMessageMap.containsKey(deliveryTag)){
            System.out.println(new Date() +" ############# enexpected, delivery tag "+deliveryTag+" not in acked messages " + ackedMessageMap + " " + _unacknowledgedMessageMap  );
        }
