                    ...
                    for (int i = 0; i < retryCount; i++) {
                        try {
                            ...
                            if (((AMQMessage)entry.getMessage()).isTopicMessage() 
                                    || ackHandler.checkAndRegisterSent(entry, deliveryTag, getChannel())) {

                                if (log.isDebugEnabled()) {
                                    ByteBuffer buf = ByteBuffer.allocate(100);
                                    int readCount = entry.getMessage().getContent(buf, 0);
                                    log.info("sent2(" + entry.getMessage().getMessageNumber() + ")"
...
//            return false;
    ...
    public void onDequeue(final QueueEntry queueEntry) {
        ...
        ClusterResourceHolder.getInstance()
                .getCassandraMessageStore().addAckedMessage(queueEntry.getMessage().getMessageNumber());
    }
