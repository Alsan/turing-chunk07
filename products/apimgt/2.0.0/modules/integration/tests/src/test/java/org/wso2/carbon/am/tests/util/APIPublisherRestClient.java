...
public class APIPublisherRestClient {
    ...
    public HttpResponse addAPI(AddAPIRequest apiRequest)
            throws Exception {
        checkAuthentication();
    ...
    public HttpResponse updateAPI(AddAPIRequest apiRequest)
            throws Exception {
        checkAuthentication();
        HttpResponse response = HttpRequestUtil.doPost(new URL(backEndUrl + URL_SURFIX + "/item-add/ajax/add.jag")
    ...
    public HttpResponse changeAPILifeCycleStatusTo(UpdateAPILifeCycleStateRequest updateRequest)
            throws Exception {
        checkAuthentication();
    ...
    public HttpResponse deleteApi(String apiName)
            throws Exception {
        checkAuthentication();
        HttpResponse response = HttpRequestUtil.doPost(new URL(backEndUrl + "/publisher/site/blocks/item-add/ajax/add.jag")
                , "action=addAPI&name=" + apiName + "&version=1.0.0&description=TwitterSearch&endpoint=" + "" + "&wsdl=&" +
                "tags=twitter,open,social&tier=Silver&context=/twitter&tiersCollection=Gold&resourceCount=0" +
                "&resourceMethod-0=POST&resourceMethodAuthType-0=Application&resourceMethodThrottlingTier-0=Gold&uriTemplate-0=/*"
                , requestHeaders);
        if (response.getResponseCode() == 200) {
            VerificationUtil.checkErrors(response);
            return response;
